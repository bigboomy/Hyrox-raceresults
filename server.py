...
def build_result(athlete_row, df, cols, season, location):
    ...
    # Scope all benchmarks to the athlete's own division so field_size, medians,
    # and percentiles are meaningful (e.g. female pro vs all female pro finishers).
    div_df = df
    if division_col and division:
        mask = df[division_col].astype(str).str.lower() == division.lower()
        scoped = df[mask]
        if len(scoped):
            div_df = scoped
    field_size = len(div_df)

    # pyrox does not expose a race date column — event_name contains year only ("2025 Melbourne")
    race_date = None

    # Compute rank within the athlete's division from sorted finish times
    rank = None
    if total_col and total is not None:
        import math as _math
        try:
            athlete_total = float(total)
            if not _math.isnan(athlete_total):
                faster = (div_df[total_col].astype(float) < athlete_total).sum()
                rank = int(faster) + 1
        except Exception:
            pass

    splits = []
    medians = div_df[split_cols].median() if split_cols else {}
    top10   = div_df[split_cols].quantile(0.10) if split_cols else {}
    ...
    benchmarks = {}
    if total_col and total is not None:
        faster_count    = (div_df[total_col] < float(total)).sum()
        percentile_rank = round((faster_count / field_size) * 100) if field_size else 0
        benchmarks = {
            "top_percent":  100 - percentile_rank,
            "median":       fmt_time(div_df[total_col].median()),
            "top_25_pct":   fmt_time(div_df[total_col].quantile(0.25)),
            "top_10_pct":   fmt_time(div_df[total_col].quantile(0.10)),
            "top_5_pct":    fmt_time(div_df[total_col].quantile(0.05)),
            "gap_to_median": fmt_delta(float(total) - float(div_df[total_col].median())),
            "gap_to_top_10": fmt_delta(float(total) - float(div_df[total_col].quantile(0.10))),
        }
...
@app.get("/hyrox/columns")
def hyrox_columns(season: int, location: str):
    """Return raw column names for a race — useful for debugging date/rank detection."""
    try:
        df = get_race_df(season, location)
        sample = {c: str(df[c].iloc[0]) if len(df) else None for c in df.columns}
        return {"columns": list(df.columns), "sample_row": sample, "rows": len(df)}
